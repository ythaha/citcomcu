 ...
 *    a lot of routines.
 * ===================================================== */
// TODO ??? the first three d.o.f. in Node_map are for the current node
// the other d.o.f. in Node_map are for nodes with smaller node number less the current node
				...
				for(kk = 1; kk <= noz; kk++)
				{
					...
					for(i = is; i <= ie; i++)
						for(j = js; j <= je; j++)
							for(k = ks; k <= ke; k++)
							{
								ja = nn - ((2 - i) * noxz + (2 - j) * noz + 2 - k);    //NOTE nn的上下前后左右,上左，上右。。。共 (3*3*3-1)/2=13个点，13*3 + 3 = 42
				...
				{				/* j is the node we are receiving from */
					...
					if(node1 <= node)
					{
						...
						assert(found /* direction 2 */ );
							// NOTE yt : here index != index in direction 1
...
// for each element : get_elt_g (E, el, E->elt_del[lev][el].g, lev)
...
void construct_stiffness_B_matrix(struct All_variables *E)
{
	...
	if(been_here0 == 0)
	{
		for(i = E->mesh.levmin; i <= E->mesh.levmax; i++)
			...
			{
				E->elt_k[i] = (struct EK *)malloc((E->lmesh.NEL[i] + 1) * sizeof(struct EK));    // REVIEW important E->elt_k
			}
	}
	...
	if(been_here0 == 0 || (E->viscosity.update_allowed && E->monitor.solution_cycles % E->control.KERNEL == 0))
	{
		...
		construct_elt_gs(E);    // TODO yt:   ??? elt_g ?
