...
#include "advection.h"
	...
	struct ID *id;
	struct COORD *ECO[MAX_LEVELS];  // Ele coord   // E->ECO[lv][e].centre[i] = centre[i] / enodes[E->mesh.nsd];centre[i] += E->XX[lv][i][E->IEN[lv][e].node[node]]; 
	struct IEN *IEN[MAX_LEVELS];	/* global at each level */
	struct ID *ID[MAX_LEVELS];
	struct NEI NEI[MAX_LEVELS];
	struct SUBEL *EL[MAX_LEVELS];    // 每个element的点，对应更密（2*2*2）网格的该点的global node number
	...
	struct Shape_function_dA *GDA[MAX_LEVELS];  // E->GDA[level][el].vpt[k] = jacobian
