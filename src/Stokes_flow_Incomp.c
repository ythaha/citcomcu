...
void solve_constrained_flow_iterative(struct All_variables *E)
{
	...
	v_from_vector(E, E->V, E->U);  // NOTE yt: from U to V
...
*/    // NOTE yt: 对每一个monitor.solution_cycles，输出到窗口count才能小于imp
	while((count < *steps_max) && (dpressure >= imp || dvelocity >= imp))
	{
		...
		valid = solve_del2_u(E, u1, Ah, imp * v_res, lev, 1); // NOTE yt: Ah is used to get u1
		strip_bcs_from_residual(E, u1, lev);
